/* This file has been borrowed and modified from the MINIX 1.5.10
 * to also work with the ARC86 LBA bios disk access. 
 *
...
#define BIOS_READ       0x1B	/* opcode for BIOS read */
#define BIOS_WRITE      0x1C	/* opcode for BIOS write */
...
/* Carry out a read or write request from the disk. */
    ...
    Bx = (unsigned) user_phys % CLICK_SIZE;	/* low order 4 bits */
    Es = physb_to_click(user_phys);	/* high order 16 bits */

    /* set Cx to the lower 16 bits of sector */
    Cx = (unsigned) sector & 0xFFFF;
 ...
 */
  ...
  wini[0].wn_size = 256000L;
  wini[DEV_PER_DRIVE].wn_size = 0L;
